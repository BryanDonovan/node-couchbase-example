...
function create_client(settings) {
    ...
    self.CHECK_RESULTS_CODE = couchbase.errors.checkResults;
    ...
    self._is_valid_key = function (key) {
        return utils.is_number(key) || typeof key === 'string';
    };
    ...
    self.set = function (key, value, meta, cb) {
        ...
        self.connect(function (err, conn) {
            ...
            if (self._is_valid_key(key)) {
                ...
                conn.release();
                return cb(new Error('invalid key: must be a string or number'));
            }
    ...
    self.get_multi = function (keys, cb) {
        self.connect(function (err, conn) {
            if (err) { return cb(err); }

            // Weird that we have to pass in null as the second arg.
            conn.getMulti(keys, null, function (err, result) {
                conn.release();
                if (err && err.code === self.KEY_NOT_FOUND_CODE) {
                    return cb(null, null);
                }
